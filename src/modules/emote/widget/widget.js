...
const deproxy = (proxy) => JSON.parse(JSON.stringify(proxy));
...
export default class Widget extends LightningElement {
    ...
    controlAnimation() {
        if (this.lastButtonsData) {
            const prevData = this.lastButtonsData;
            const nextData = deproxy(this.buttonsData);
            const changedButtonsData = nextData.reduce((acc, next, i) => {
                const prev = prevData[i];
                const difference = +next.count - +prev.count;
                if (difference <= 0) return acc;
                const emojiUrl = emojiUrlLookup[next.name];
                return [
                    ...acc,
                    {
                        ...next,
            ...
            changedButtonsData.forEach(async ({ name, imgUrl, count }) => {
                if (!this[`${name}Image`]) {
                    const loadImg = () =>
                        new Promise((res) => {
                            const img = new Image();
                ...
                this.template
                    .querySelector('emote-animation')
                    .fireAnimation(this[`${name}Image`], count);
            });
        }
        this.lastButtonsData = deproxy(this.buttonsData);
    }
